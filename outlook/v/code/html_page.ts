...
export { mutall_error, basic_value, myalert };
...
export abstract class html_page extends view {
  //
  ...
  get hidden(): boolean {
    ...
    const hidden: boolean | undefined = this.options?.hidden;
    //
  ...
  //(a.k.a. listeners).
  public get_input_value<
    //
    //Let q be the required type. If yes, the return value should string|Error
    //If not, then the return is string|null|Error
    q extends "yes" | "no",
    //
    //The action to take if there is an error. If the result is erroneous and
    //return type is throw,then we throw an exception
    a extends "return" | "throw",
    //
    //The general return value is string | null | Error.  When a value is
    //required the return value is generally  string | Error
    r extends q extends "yes"
      ? //
        //When a value is required, the general return value is refined t
        //a string
        a extends "throw"
        ? string
        : string | Error
      : //
        //The generl ratien value
        string | null | Error
  >(id: string, required: q = <q>"yes", action: a = <a>"return"): r {
    //
    ...
    const value: null | string = elem.value.trim() === "" ? null : elem.value;
    //
    //Compile a required error
    const err: Error = new Error(`${id} is required`);
    //
    //The next course of action is determined by whether the value is null
    //or not
    //
    if (!value && required === "yes" && action === "throw") throw err;
    //
    if (!value && required === "yes" && action === "return") return <r>err;
  ...
  myget<i>(x: i | undefined, name?: string): i {
    ...
    //...otherwise report it
    throw new mutall_error(
      `Variable ${name ?? ""} not set. Check initialization`
    );
  }
  ...
  get_files_using_envelop(env: HTMLElement): FileList | null {
    ...
    if (inputs.length === 0)
      throw new mutall_error(
        `No file input element is found under current envelop element`,
        env
      );
    //
    ...
    if (inputs.length > 1)
      throw new mutall_error(
        `There is more than 1 file input element under curremt envelop`,
        env
      );
    //
  ...
  get_envelop_element(id: string): HTMLElement {
    ...
    const elements: Array<HTMLElement> = Array.from(
      this.document.getElementsByName(id)
    );
    if (elements.length > 0) {
      //
      const data_field: HTMLElement | null = elements[0].closest(
        "*[data-field], *[id]"
      );
      //
    ...
    //Element not found
    throw new mutall_error(
      `No envelop element matches '${id}' using the Name, Id or Data-field strategy`
    );
  }
  ...
  private get_radio_value(env: HTMLElement): string | null {
    ...
    if (!name)
      throw new mutall_error(
        `The envlop enclosing radio buttons must have a data field named the same as the buttons`,
        env
      );
    //
  ...
  get_todays_date(): string {
    ...
    const formatted_date: string = `${year}-${formatted_month}-${formatted_day}`;
    //
