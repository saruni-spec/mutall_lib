import {panel, page} from "./outlook.js";
//
import {cname, database, mutall_error, basic_value, fuel} from "../../../schema/v/code/schema.js";
//
import {io} from "../../../schema/v/code/io.js";
//
import {app} from './app.js';
//
import{position, pk} from "../../../schema/v/code/library";
...
export class theme extends panel {
    /** 
    ...
    public async continue_paint() {
        ...
        const metadata = await this.exec_php(
            //
        ...
        this.dbase = new database(idbase);
        //
    ...
    protected initialize_styles() {
        ...
        const head = this.parent.document.querySelector('head');
        if (head===null) 
            throw new mutall_error('Head element not found');
        //
   ...
   //The primary key column will also serve as the multi line selector
    ...
    private async execute_outcome(outcome: outcome, request: offset) {
        ...
        switch (outcome.type) {
            ...
            case "fresh":
                ...
                if (tbody === null)
                    throw new mutall_error("tbody not found");
                // 
            ...
            default:
                throw new mutall_error(`The outcome of type 
    ...
    protected async load_body(offset: offset/*:int*/, dir: direction/*:mytop | bottom*/) {
        ...
        const constrained_limit = h < app.current.config.limit ? h : app.current.config.limit;
        //
        //Query the database 
        const result: Array<fuel> = await this.query(offset, constrained_limit);
        //
        //   
    ...
    private async query(offset: offset, limit: number): Promise<Array<fuel>>{
        // 
    ...
    public load_tr_element(
        ...
        const pairs: Array<[string, basic_value]> = row === undefined
        ...
        pairs.forEach(async([key, value]) => {
            ...
            const Io = await this.create_io(td);
            //
            //Paint the io
            Io.show();
            //
    ...
    private async create_io(td: HTMLTableCellElement): Promise<io> {
        // 
        ...
        //its io type.
        const Io = await io.create_io(td, undefined, this.parent, {col});
        // 
        return Io;
    }
    ...
    protected select_nth_row(pk?: pk) {
        // 
    ...
    async goto(request?: offset) {
        ...
        if (request < this.extreme.top)
            throw new mutall_error(`Goto: A request ${request}
             must be positive`);
        //
