...
export abstract class quiz<i> extends view {
    ...
    //Doing it here means that we still have access to the dialog that was used 
    //for data collection so that if the execute fails we can edit the data and
    //re-execute this method. Here are 2 examples of execute:-
    ...
    protected async open(): Promise<{ submit: HTMLElement; cancel: HTMLElement }> {
        ...
        const buttons:{submit:HTMLElement, cancel:HTMLElement} = this.get_navigation_buttons();
        //
        return buttons;
    }
    ...
    //Consider constructing them if they are not available
    get_navigation_buttons():{submit:HTMLElement, cancel:HTMLElement}{
        //
        const [submit, cancel]= ['submit', 'cancel'].map(id=>{
            //
            //There must be a proxy for this quiz
            if (!this.proxy) throw new mutall_error(`No proxy found for this view`);
            //
            //Get any element by the the id, starting the search from the proxy
            //element
            const elem:HTMLElement|null = this.proxy.querySelector(`#${id}`);
            //
            ...
            const elem2:HTMLElement = this.create_element('button', this.proxy,{
                textContent:id
            });
            //
            return elem2; 
    ...
    private async show(url: string): Promise<void> {
        ...
        if (!this.proxy) throw new mutall_error(`Proxy element for this view is not defined `)    
        //
