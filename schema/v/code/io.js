import { mutall_error, foreign as schema_foreign, view, } from "./schema.js";
//
...
export class io extends view {
    ...
    options) {
        ...
        this.output = this.create_element("span", this.proxy, {
            className: "normal",
        });
        //
    ...
    myoptions) {
        ...
        if (col instanceof primary)
            return new primary(proxy, undefined, undefined, parent, options);
        //
        //Foreign key columns have a matching io
        if (col instanceof schema_foreign)
            return new foreign_io(proxy, col, parent, options);
        //
        ...
        if (col.length >= 100)
            return new textarea(proxy, undefined, parent, options);
        //
        ...
        if (["date", "time", "datetime"].find((dtype) => dtype === col.data_type))
    ...
    ...elements) {
        ...
        const header = text instanceof HTMLElement ? text : this.document.createTextNode(text);
    ...
    options) {
        ...
        const result = io.get_io_type(proxy, options, parent, type);
        //
        ...
        switch (result) {
            ...
            case "number":
            case "email":
                return new input(proxy, result, parent, options);
            //
            //For the more sophisticated io types....
            //
            //A single checkbox used for representing a boolean io
            case "checkbox":
                return new checkbox(proxy, parent, options);
            //
            //Single or multiple choices implemented as a set of radio or
            //checkbox inputs
            case "checkboxes":
                return new choices(proxy, "multiple", parent, options);
            case "radios":
                return new choices(proxy, "single", parent, options);
            //
            ...
            //Working with images
            case "image":
                return new image(proxy, parent, options);
            //
    ...
    static get_io_type(proxy, options, parent, type) {
        //
        ...
        if (options?.col)
            return io.deduce_io(proxy, options.col, parent, options);
        //
    ...
    get error() {
        ...
        const tag = this.proxy.querySelector(".error");
        //
        //If not  found, throw an excepetion
        if (tag === null)
            throw new mutall_error(`Cannot fine the error reporting tag for this io`, this);
        //
        return tag;
    }
}
...
export class select extends io {
    ...
    parent, options) {
        ...
        if (!col)
            throw new mutall_error(`No choices found for this selector`);
        //
        //Get the choices from the column attribute.
        const choices = this.get_choices(col.type);
        //
        //Add the choices to the selector
        choices.forEach((choice) => this.create_element("option", this.input, {
            value: choice,
            textContent: choice,
            id: choice,
        }));
...
export class url extends io {
    ...
    //
    constructor(proxy, parent, options) {
        super(proxy, "url", parent, options);
        //
        this.output = this.create_element(`a`, this.proxy, { className: "normal" });
        //
        //Create a the url label
        const url_label = this.create_element(`label`, this.proxy, { className: "edit", textContent: "Url Address: " });
        //
        ...
        //
        //Create a text label
        const text_label = this.create_element(`label`, this.proxy, {
            className: "edit",
...
export class input extends io {
    ...
    parent, options) {
        ...
        //it is defined)
        if ((x = this.get_maxlength()))
            this.input.maxLength = x;
    }
    ...
    get_size() {
        ...
        const len = this.search_option("size");
        if (len)
            return len;
        //
    ...
    get_maxlength() {
        ...
        const len = this.search_option("maxlength");
        if (len)
            return len;
        //
        //Search for a column in the options, returning its length if found
        if (this.options?.col)
            return this.options.col.length;
        //
...
export class radio extends io {
    ...
    //
    constructor(name, checked, proxy, parent, options) {
        //
...
export class file extends input {
    ...
    //
    constructor(proxy, type, parent, options) {
        //
        ...
        this.create_element("option", this.source_selector, {
            value: "local",
    ...
    show() {
        ...
        if (this.image !== undefined)
            this.proxy.appendChild(this.image);
        //
...
export class textarea extends input {
    ...
    textarea;
    //
    constructor(proxy, size, parent, options) {
        //
...
class choices extends io {
    ...
    //
    constructor(proxy, type = "multiple", parent, options) {
        //Determine the type of choices
        const i = type === "single" ? "radios" : "checkbox";
        //
        super(proxy, i, parent, options);
        this.type = type;
        //Query select all the input choices of this io, converting them into
        //an array
        this.choices = (Array.from(this.proxy.querySelectorAll("input")));
    }
    ...
    set input_value(value) {
        ...
        //For the single choices....
            ...
            const input = this.choices.find((input) => input.value === str);
            //
    ...
    get input_value() {
        ...
        const choices = (Array.from(this.proxy.querySelectorAll("input:checked")));
        //
        //For the multiple choices...
        if (this.type === "multiple") {
            //
...
export class foreign_io extends io {
    ...
    options) {
        ...
        //the normal one
        this.friendly = this.create_element(`span`, this.proxy, {
            className: "normal friendly",
        });
        //
        //Add to this foreign io, a button for initiating editing.
        this.button = this.create_element(`input`, this.proxy, {
            type: "button",
            //
    ...
    async show_dialog() {
        ...
        this.create_element("button", this.dialog, {
            textContent: "Cancel",
    ...
    submit() {
        ...
        if (selection === undefined)
            throw new mutall_error(`Please select a record`);
        //
