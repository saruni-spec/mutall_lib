...
//
//References to the original questiopnnaire
...
//
//A concrete type is one than can be used for constructing a instances
export type concrete = new(...args:any)=>any;
//
//In contrast, an abstract class cannot be used for constructing any class
export type $abstract = abstract new(...args:any)=>any;
//
//The database class referenced in a tuple is a dictionary of concrete entuty classes
//indexed by their entity names
export type database = {[ename:schema.ename]: concrete};
//
//The list of databases on a server is a dictionary of databases indexed by 
//their names
const databases:{[dbname:schema.dbname]:database} = {
    //
...
class msg extends tuple{
    constructor( 
        ...
        super('mutall_user', 'msg', {type:'ready', data:{msg, etc}});
      ...
    }
    ....    
}
...
//User-editable properties are defined as those that are nullable, i.e., 
//{
//  msg:value, 
//  date:Date, 
//  text:string,
//
////NB. business and user are not basic types, but they are editable
//  user:user,
//  business:business,
//
//  language?:string,
//  child_of?:pointer<msg>
// } 
//Note that to ensure that properties from the base entity are not considered 
//for editing we should deliberately ensure that they are not nullable. This
//overtaken by events. We use property entity to separate the tuple's data from
//its metadata
export type edit<Tuple extends tuple> ={
    //
    //If null can be assigned to a value, then that value is editable
    //Also note that the value of an editable column excludes a null; otherwise 
    //it has the same data type as he original value 
    [Col in keyof Tuple as null extends Tuple[Col]  ? Col:never]: Exclude<Tuple[Col], null>
}

    
//The basic type of a tuple is designed to match the structure of data
//returned from a database, before it becomes editable, i.e., usable for 
//programming. It has 2 parts. One part is designed to hold simple scalars of 
//a tuple, a.k.a., fuel. The other part hold matrices, i.e., tuples pointing to
//this basic one
//Given our example, the desired output is:-
//{
////The scalars part    
//  msg:friend, /*that can be decoded to a friend, i.e., {value:number, friend:string}*/
//  date:string,
//  text:string, 
//  user:friend /*that can be decoded to a friend*/, 
//  business:friend /*that can be decoded a friend*/
//
////The matrix part. NB. The foreign key field (not entity) is used as the basic 
//key in order to support hierarchical relationships   
//  child_of:Array<scalars<msg>>
//  }
//export type basic<T extends tuple> = reader<T> & matrices<T>;

//The scalar object (derived from a tuple) comprises of editable values that 
//are not arrays.
...
export  class pointer<T extends tuple>{
    ...
    async populate():Promise<Array<T>>{
        ...
        const factory:new(...args:any)=>any = databases[dbname][ename];
        //
...
export abstract class tuple{
    ...
    constructor(
        ...
        public dbname:schema.dbname,
        //
        //The entity name
        public ename: schema.ename,
        //
    ...
    //Collect all the layouts of this entity
    abstract collect_layouts(): Generator<quest.layout>;
        ...
        for(const dbname in schema.databases){
            ...
            for(const ename in dbase.entities){
                ...
                for(const cname in dbase.entities[ename].columns){
                    ...
                    if (col.ref.dbname !== this.dbname) continue;
                    if (col.ref.ename !== this.ename) continue;
                    //
                    //Collect this column
                    yield col;
                }
