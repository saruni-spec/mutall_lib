...
class school_panel extends panel {
    ...
    async display_1st_class_(school_pk) {
        // 
   ...
   `;
        ...
        const results = await this.exec_php('database', ['school', false], 'get_sql_data', [sql]);
        // 
        ...
        if (results.length === 0) {
            ...
            const school_page = this.heterozone.parent;
            //
            ...
            //
            // 
            const school_panel = school_page.school_panel;
            // // 
        ...
        const pk = results[0]['school.school'];
        // 
        ...
        const school = this.heterozone.parent;
        //
        //4.1.2 Ensure that the school page is indeed a school page
...
class class_ extends panel {
    ...
    //   
    // Displays the first stream for the class thats clicked on
    async display_1st_stream(class_pk) {
        // 
        // 1.Formulate the sql(ist the one in the file)
        const sql = `
        select
       ...
       `;
        ...
        const pk = results[0]['class.class'];
        // 
        //4.Select the entry from  student panel that has the result pk 
        // 4.1 Get the student panel
        //
        ...
        // 
        // 4.2 get the cell from the student panel that has primary key /school id coordonates
        const cell = stream.cells_indexed[String(pk)]['school.id'];
        // 4.3 Select the cell
        cell.select();
        // 
        // show the selection
...
class stream extends panel {
    ...
    async display_1st_year(stream_pk) {
        // 
        // 1.Formulate the sql(ist the one in the file)
        const sql = `
         select
       ...
       `;
        ...
        // 
        // 4.2 get the cell from the student panel that has primary key /school id coordonates
        const cell = year.cells_indexed[String(pk)]['school.id'];
        // 4.3 Select the cell
        cell.select();
        // 
        // show the selection
...
class year extends panel {
    ...
    async display_1st_student(year_pk) {
        // 
        // 1.Formulate the sql(ist the one in the file)
        const sql = `
        select
        ...
        const pk = results[0]['progress.progress'];
        // 
        //4.Select the entry from  student panel that has the result pk 
        // 4.1 Get the student panel
        //
        ...
        const student = school.student;
        // 4.2 get the cell from the student panel that has primary key /school id coordonates
        const cell = student.cells_indexed[String(pk)]['school.id'];
        // 4.3 Select the cell
        cell.select();
        // 
        // show the selection
